...
# default dict for net
# defeats duplicates required?
    ...
    def enabled(self, nodelist):
        ...
        for req in self.reqs:
            if isinstance(req, tuple):
                if not all_in(req, nodelist):
                    return False
            else:
                if not req in nodelist:
                    return False
        return True
...
class Ability(Node):
    ...
    def eclipses(self, ability):
        """Check and see if I eclipse ability.
        If I do, those abilities are required in my immediate past.
        """
        # i eclipse an ability if, for every obstacle it can
        # defeat, i can defeat it too
        if len(self.defeats) <= len(ability.defeats):
            return
        if all([d in self.defeats for d in ability.defeats]):
            self.reqs.add(ability.name)
...
class Network(object):
    ...
    def calc_ability_eclipses(self):
        ...
        for (a, b) in itertools.combinations(self.abilities.values(), 2):
            a.reqs = set()
            a.eclipses(b)
    ...
    def add_connection(self, node1, node2):
        ...
        # node2 is either enabled from node1, or placed
        assert node2 in enabled or node2 in self.net
        # initialize node2 unlocks
    ...
    def enabled_nodes(self, branch, suppress_placed=False):
        ...
        if suppress_placed:
            placed_nodes = [
                node for node in enableds if self.nodes[node].placed]
    ...
    def locked_abilities(self):
        ...
        """
        # TODO: couldnt this be just ability.enabled or smth?
    ...
    def active_abilities(self):
        ...
        print [node for node in self.abilities if self.abilities[node].placed]

    def active_obstacles(self):
        """Print all the active obstacles in the net"""
        print [node for node in self.obstacles if self.obstacles[node].placed]
    ...
    def new_obstacles(self):
        ...
        for ab in [n for n in self.abilities if len(self.nodes[n].reqs) > 1 and n not in self.net]:
            ...
            if all(reqs_met):
                ...
                print '{} requires a joint obstacle, possibly with the following requirements:'.format(
                    ab)
                for joint in sets:
                    print joint
                print '\n'
